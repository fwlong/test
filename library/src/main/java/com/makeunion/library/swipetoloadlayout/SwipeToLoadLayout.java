...
public class SwipeToLoadLayout extends ViewGroup {
    ...
    // 下拉，没有达到触发刷新的阈值，弹回去的时间
    private static final int DEFAULT_SWIPING_TO_REFRESH_TO_DEFAULT_SCROLLING_DURATION = 300;

    // 下拉刷新，松手后，从当前位置滚动到“刷新中”位置的时间
    private static final int DEFAULT_RELEASE_TO_REFRESHING_SCROLLING_DURATION = 300;

    // 下拉刷新成功后，header弹上去的延迟时间，正常应该为0，为了有一个停顿的效果，设置为200
    private static final int DEFAULT_REFRESH_COMPLETE_DELAY_DURATION = 200;

    // 下拉刷新成功后，header弹上去的时间
    private static final int DEFAULT_REFRESH_COMPLETE_TO_DEFAULT_SCROLLING_DURATION = 500;

    // 自动刷新时，弹出header的时间
    private static final int DEFAULT_DEFAULT_TO_REFRESHING_SCROLLING_DURATION = 300;

    //**********************************************************************************************

    // 上拉，没有达到触发加载更多的阈值，弹回去的时间
    private static final int DEFAULT_SWIPING_TO_LOAD_MORE_TO_DEFAULT_SCROLLING_DURATION = 300;

    // 上拉加载，松手后，从当前位置滚动到“加载中”位置的时间
    private static final int DEFAULT_RELEASE_TO_LOADING_MORE_SCROLLING_DURATION = 300;

    // 上拉加载成功后，footer弹下去的延迟时间，正常应该为0，为了有一个停顿的效果，设置为200
    private static final int DEFAULT_LOAD_MORE_COMPLETE_DELAY_DURATION = 200;

    // 上拉加载成功后，footer弹下去的时间
    private static final int DEFAULT_LOAD_MORE_COMPLETE_TO_DEFAULT_SCROLLING_DURATION = 500;

    // 自动加载更多时，弹出footer的时间
    private static final int DEFAULT_DEFAULT_TO_LOADING_MORE_SCROLLING_DURATION = 300;

    private static final long DEFAULT_ANIMATION_EXECUTION_TIME = 1200;

    // 页面拖拽距离和手指滑动距离比例，越小，阻尼效果越强
    ...
    private static final float DEFAULT_HEADER_VIEW_REFRESHING_SINK = 0f;
    ...
    /**
     * <b>ATTRIBUTE<b/>
     * set header view sink distance;
     */
    private float mHeaderViewSink;
    ...
    //设置刷新headerview和footview背景色
    protected int mSwipeBackgroundColor = Color.WHITE;
    ...
    public SwipeToLoadLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SwipeToLoadLayout,
                defStyleAttr, 0);
        try {
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (attr == R.styleable.SwipeToLoadLayout_refresh_enabled) {
                    ...
                    setLoadMoreTriggerOffset(a.getDimensionPixelOffset(attr, 0));
                        ...
                        == R.styleable
                        .SwipeToLoadLayout_swiping_to_refresh_to_default_scrolling_duration) {
                    setSwipingToRefreshToDefaultScrollingDuration(a.getInt(attr,
                            DEFAULT_SWIPING_TO_REFRESH_TO_DEFAULT_SCROLLING_DURATION));
                    ...
                    setRefreshCompleteDelayDuration(
                        ...
                        == R.styleable
                        .SwipeToLoadLayout_refresh_complete_to_default_scrolling_duration) {
                    ...
                    setDefaultToRefreshingScrollingDuration(
                        ...
                        == R.styleable
                        .SwipeToLoadLayout_swiping_to_load_more_to_default_scrolling_duration) {
                    setSwipingToLoadMoreToDefaultScrollingDuration(a.getInt(attr,
                            DEFAULT_SWIPING_TO_LOAD_MORE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr
                        == R.styleable
                        .SwipeToLoadLayout_release_to_loading_more_scrolling_duration) {
                    ...
                    setLoadMoreCompleteDelayDuration(
                        ...
                        == R.styleable
                        .SwipeToLoadLayout_load_more_complete_to_default_scrolling_duration) {
                    setLoadMoreCompleteToDefaultScrollingDuration(a.getInt(attr,
                            DEFAULT_LOAD_MORE_COMPLETE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr
                        == R.styleable
                        .SwipeToLoadLayout_default_to_loading_more_scrolling_duration) {
                    setDefaultToLoadingMoreScrollingDuration(
                            a.getInt(attr, DEFAULT_DEFAULT_TO_LOADING_MORE_SCROLLING_DURATION));

                }else if(attr == R.styleable.SwipeToLoadLayout_header_refreshing_sink){
                    setHeaderViewSink(a.getDimensionPixelOffset(attr,0));
                }else if(attr == R.styleable.SwipeToLoadLayout_swipe_loading_background){
                    mSwipeBackgroundColor = a.getColor(attr, Color.WHITE);
                }
            }
        } finally {
            a.recycle();
    ...
    protected void onFinishInflate() {
        ...
        if (childNum == 0) {
            ...
            if (mFooterView instanceof ViewGroup) {
                if (((ViewGroup) mFooterView).getChildCount() > 1) {
                    mNoMoreDataView = ((ViewGroup) mFooterView).getChildAt(1);
                }
            }
        } else {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (STATUS.isSwipingToRefresh(mStatus) || STATUS.isSwipingToLoadMore(mStatus) ||
                    ...
                    if (mDebug) {
                        Log.i(TAG,
                                "Another finger down, abort auto scrolling, let the new finger "
                                        + "handle");
                    }
                }

                if (STATUS.isSwipingToRefresh(mStatus) || STATUS.isReleaseToRefresh(mStatus)
                        || STATUS.isSwipingToLoadMore(mStatus) || STATUS.isReleaseToLoadMore(
                        mStatus)) {
                    return true;
                }
                ...
                //如果正在刷新，且headerview处于完全不可见和完全可见之间，滑动可移动headerview
                if (STATUS.isRefreshing(mStatus)) {
                    if (mTargetOffset > 0 && mTargetOffset <= mRefreshTriggerOffset) {
                        return true;
                    } else {
                        return false;
                    }
                ...
                //如果正在加载，footerview处于完全可见和完全不可见之间，滑动可移动footerview
                if (STATUS.isLoadingMore(mStatus)) {
                    if (mTargetOffset < 0 && (-mTargetOffset <= mLoadMoreTriggerOffset)) {
                        return true;
                    } else {
                        return false;
                    }
                ...
                // SwipeToLoadLayout#onTouchEvent() ACTION_DOWN return true to consume the
                // ACTION_DOWN event.
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (triggerCondition) {
                    ...
                    // intercept the move action event and pass it to
                    // SwipeToLoadLayout#onTouchEvent()
                    return true;
                }
                boolean doingCondition =
                        (yInitDiff > 0 && moved && STATUS.isLoadingMore(mStatus)
                                && mTargetOffset < 0)
                                || (yInitDiff < 0 && moved && STATUS.isRefreshing(mStatus)
                                && mTargetOffset > 0);
                if (doingCondition) {
                    return true;
                }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (STATUS.isStatusDefault(mStatus)) {
                ...
                } else if (STATUS.isRefreshStatus(mStatus) && !STATUS.isRefreshing(mStatus)) {
                    if (mTargetOffset <= 0) {
                        setStatus(STATUS.STATUS_DEFAULT);
                        fixCurrentStatusLayout();
                        //解决手指从refresh滑动到default的位置仍然不松时，headerview没重置，状态变为loadmore，导致后面headerview 不消失
                        mRefreshCallback.onReset();
                        return false;
                    }
                } else if (STATUS.isLoadMoreStatus(mStatus) && !STATUS.isLoadingMore(mStatus)) {
                    if (mTargetOffset >= 0) {
                        setStatus(STATUS.STATUS_DEFAULT);
                        fixCurrentStatusLayout();
                        //解决手指从loadmore滑动到default的位置仍然不松时，loadmoreview 没重置，状态变为loadmore，导致后面loadmoreview 不消失
                        mLoadMoreCallback.onReset();
                        return false;
                    }
                } else if (STATUS.isRefreshing(mStatus)) {
                    //刷新时，上拉避免footerviewoffset超过0，也导致了刷新时不可加载
                    if (yDiff < 0) {
                        if (mTargetOffset < 0) {
                            mTargetOffset = 0;
                            layoutChildren();
                    ...
                    //加载时，下拉避免将headerviewoffset超过0，也导致了加载时不可刷新
                    if (yDiff > 0) {
                        if (mTargetOffset > 0) {
                            mTargetOffset = 0;
                            layoutChildren();
                ...
                if (STATUS.isRefreshStatus(mStatus)) {
                    ...
                    if (STATUS.isSwipingToLoadMore(mStatus) || STATUS.isReleaseToLoadMore(
                            mStatus)) {
    ...
    /**
     * set refresh header view, the view must at lease be an implement of {@code
     * SwipeRefreshTrigger}.
    ...
    /*
    * set {@link #mHeaderViewSink}
    */
    public void setHeaderViewSink(float distance){
        mHeaderViewSink = distance;
    }

    public float getHeaderViewSink(){
        return mHeaderViewSink;
    }
    ...
    public void setRefreshing(boolean refreshing) {
        ...
        if (refreshing) {
            ...
            if (STATUS.isRefreshing(mStatus)) {
                ...
                long compensateTime = 0;
                if(executionTime<DEFAULT_ANIMATION_EXECUTION_TIME){
                    compensateTime = DEFAULT_ANIMATION_EXECUTION_TIME - executionTime;
                ...
                }, compensateTime==0?mRefreshCompleteDelayDuration:compensateTime);
            }
        }
    }
    ...
    public void setLoadingMore(boolean loadingMore) {
        ...
        if (loadingMore) {
            ...
            if (STATUS.isLoadingMore(mStatus)) {
                ...
                }, compensateTime==0?mLoadMoreCompleteDelayDuration:compensateTime);
            }
        }
    }
    ...
    public void setNoData(boolean isNoData) {
        ...
        if(isNoData){
            setLoadingMore(!isNoData);
        }
        this.mIsNoData = isNoData;
        mNoMoreDataView.setVisibility(isNoData ? View.VISIBLE : View.GONE);
        //footerview中其他view应该是不可见的
        if(mFooterView instanceof ViewGroup){
            int count = ((ViewGroup) mFooterView).getChildCount();
            for(int index = 0;index<count;index++){
                if(index!=1){//index 为1 被认为是没有更多的提示view
                    ((ViewGroup) mFooterView).getChildAt(index).setVisibility(isNoData? View.GONE: View.VISIBLE);
                }
            }
        }
    }
    ...
    private void layoutChildren() {
        ...
        if (mHeaderView != null) {
            ...
            switch (mStyle) {
                case STYLE.CLASSIC:
                    // classic
                    headerTop = paddingTop + lp.topMargin - mHeaderHeight + mHeaderOffset+(mTargetOffset==0?0:(int)mHeaderViewSink);
                    break;
                case STYLE.ABOVE:
                    // classic
                    headerTop = paddingTop + lp.topMargin - mHeaderHeight + mHeaderOffset+(mTargetOffset==0?0:(int)mHeaderViewSink);
        ...
        if (mFooterView != null) {
            ...
            switch (mStyle) {
                case STYLE.CLASSIC:
                    // classic
                    footerBottom = height - paddingBottom - lp.bottomMargin + mFooterHeight
                            + mFooterOffset;
                    break;
                case STYLE.ABOVE:
                    // classic
    ...
    private void fingerScroll(final float yDiff) {
        ...
        if (STATUS.isRefreshing(mStatus) || STATUS.isLoadingMore(mStatus)) {
            yScrolled *= 2;
        }
        ...
        if (STATUS.isRefreshStatus(mStatus) && !STATUS.isRefreshing(mStatus)) {
    ...
    private void onActivePointerUp() {
        if (STATUS.isSwipingToRefresh(mStatus)) {
            ...
            if (-mTargetOffset > mFooterHeight) {
                scrollReleaseToLoadMoreToLoadingMore();
            }
        }
    }
    ...
    private void scrollDefaultToRefreshing() {
        mAutoScroller.autoScroll((int) (mRefreshTriggerOffset + 0.5f),
                mDefaultToRefreshingScrollingDuration);
    }

    private void scrollDefaultToLoadingMore() {
        mAutoScroller.autoScroll(-(int) (mLoadMoreTriggerOffset + 0.5f),
                mDefaultToLoadingMoreScrollingDuration);
    }
    ...
    private void scrollReleaseToRefreshToRefreshing() {
        mAutoScroller.autoScroll(mHeaderHeight - mHeaderOffset,
                mReleaseToRefreshToRefreshingScrollingDuration);
    }

    private void scrollReleaseToLoadMoreToLoadingMore() {
        mAutoScroller.autoScroll(-mFooterOffset - mFooterHeight,
                mReleaseToLoadMoreToLoadingMoreScrollingDuration);
    }
    ...
    private void setStatus(int status) {
        mStatus = status;
        if (STATUS.isLoadingMore(status)) {
            isLoading = true;
        } else if (STATUS.isRefreshing(status)) {
            isRefreshing = true;
        } else {
            isLoading = false;
            isRefreshing = false;
